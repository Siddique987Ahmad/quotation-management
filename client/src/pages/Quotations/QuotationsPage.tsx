  ...
  try {
    ...
    const dynamicFormData: Record<string, any> = formData.dynamicFields.reduce((acc: Record<string, any>, field) => {
      if (field.label && field.value !== undefined) {
        // If this is a scope of work field, ensure all items have category and title
        if (field.type === 'scope-of-work' && field.value?.items && Array.isArray(field.value.items)) {
          console.log('🔍 Normalizing scope of work items before submission...');
          console.log('📦 Original items:', field.value.items);
          console.log('📋 Full field.value structure:', field.value);
          console.log('📋 Current field metadata:', {
            currentCategory: field.value.currentCategory,
            currentTitle: field.value.currentTitle
          });
          
          // Get current category/title from field metadata (set when user changes them)
          // If metadata is missing, try to derive from the first item that has a title
          let currentCategory = field.value.currentCategory;
          let currentTitle = field.value.currentTitle;
          
          // If metadata is missing, try to get from items
          if (!currentCategory || !currentTitle) {
            const firstItemWithTitle = field.value.items.find((item: any) => 
              item.category && (item.servicesTitle || item.supplyTitle)
            );
            
            if (firstItemWithTitle) {
              currentCategory = currentCategory || firstItemWithTitle.category;
              currentTitle = currentTitle || (firstItemWithTitle.category === 'supply' 
                ? firstItemWithTitle.supplyTitle 
                : firstItemWithTitle.servicesTitle) || '';
            }
          }
          ...
          console.log('📋 Resolved category/title:', {
            currentCategory,
            currentTitle,
            source: field.value.currentCategory ? 'metadata' : (field.value.items.find((i: any) => i.category) ? 'items' : 'default')
          });
          
          const normalizedItems = field.value.items.map((item: any, index: number) => {
            console.log(`🔍 Item ${index} before normalization:`, {
              category: item.category,
              servicesTitle: item.servicesTitle,
              supplyTitle: item.supplyTitle
            });
            
            // Check if item is missing category/title OR if it needs to be updated to match current form metadata
            const itemNeedsUpdate = !item.category || 
                                   (!item.servicesTitle && !item.supplyTitle) ||
                                   (currentCategory && item.category !== currentCategory) ||
                                   (currentTitle && currentTitle.trim() !== '' && 
                                    ((currentCategory === 'services' && item.servicesTitle !== currentTitle.trim()) ||
                                     (currentCategory === 'supply' && item.supplyTitle !== currentTitle.trim())));
            
            if (itemNeedsUpdate) {
              // Prioritize currentCategory from form metadata over item's existing category
              // This ensures that when user changes category/title in form, all items get updated
              let defaultCategory = currentCategory || item.category;
              
              // If we have a currentCategory but item has different category, use currentCategory
              if (currentCategory && item.category && item.category !== currentCategory) {
                defaultCategory = currentCategory;
              }
              
              // Fallback to item's category or other items if no metadata
              if (!defaultCategory) {
                const firstItemWithTitle = field.value.items.find((i: any) => 
                  i.category && (i.servicesTitle || i.supplyTitle)
                );
                defaultCategory = firstItemWithTitle?.category || 'services';
              }
              
              let defaultTitle = '';
              
              // Prioritize currentTitle from form metadata
              if (currentTitle && currentTitle.trim() !== '') {
                defaultTitle = currentTitle.trim();
              } else if (item.servicesTitle || item.supplyTitle) {
                // Use item's existing title if available
                defaultTitle = defaultCategory === 'supply' 
                  ? (item.supplyTitle || '') 
                  : (item.servicesTitle || '');
              } else {
                // Try to get from other items
                const firstItemWithTitle = field.value.items.find((i: any) => 
                  i.category && (i.servicesTitle || i.supplyTitle)
                );
                if (firstItemWithTitle) {
              ...
              const normalized = {
                ...
                servicesTitle: defaultCategory === 'services' ? defaultTitle : (item.servicesTitle || ''),
                supplyTitle: defaultCategory === 'supply' ? defaultTitle : (item.supplyTitle || '')
              };
              
              console.log(`✅ Item ${index} normalized:`, {
                category: normalized.category,
                servicesTitle: normalized.servicesTitle,
                supplyTitle: normalized.supplyTitle,
                source: currentTitle && currentCategory ? 'form metadata' : (item.servicesTitle || item.supplyTitle ? 'item existing' : 'default')
              });
              
              return normalized;
            }
            
            console.log(`✓ Item ${index} already has category/title, keeping as-is`);
            return item;
          });
